...
價格模型任一檢查未通過時，正式 `price_forecast` 中央價格與區間仍維持
`null`／「未產生」。使用者於 2026-09-01 確認後，可另外顯示獨立
`research_scenario`：包含原始研究估計、情境進場、情境停損與兩段情境停利。
這些數字必須標示「未驗證、不可交易、不是買賣建議」；正式 `action`
仍為「不交易」，`execution_plan` 與 SQLite 正式買賣價位仍為空值。

`20260828.4` 的純研究價格模型會在每個 fold 的訓練窗口內再切出尾端最多 126 個交易日
作時間順序校準。候選固定為零報酬、歷史平均、Ridge，以及收縮係數
`0.10／0.25／0.50／0.75／1.00` 的 ExtraTrees；先要求校準期方向準確率至少 52%，
再以校準期 MAE 選擇。80% 區間改用校準期絕對誤差的 Split Conformal 分位數。
選擇與區間校準均不得讀取 fold 測試結果。這套研究模型不參與交易訊號、進出場或
正式交易價位計算。
...
.venv/bin/python scripts/predict.py 00631L.TW \
  --period max --folds 5 \
  --context 0050.TW 2330.TW ^TWII \
  --label-mode legacy-target --no-record
```
...
未驗證研究情境另外以 prediction ID 一對一寫入
`prediction_research_scenarios`，包含原始研究估計與區間、情境進場與區間、
情境停損、兩段情境停利及失敗驗證。這些數值不會寫入正式買賣欄位，
也不會被 outcome 視為真實交易。
