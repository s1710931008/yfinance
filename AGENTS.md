...
使用者於 2026-09-01 明確確認一項有限例外：必要行情與計算均成功、但模型驗證未通過時，
可以另外顯示「未驗證研究情境價位」。此例外不適用於資料不足、行情無法確認或程式失敗，
並且必須同時符合：

- 正式結論與 `action` 維持「不交易」，不得因研究情境改為買進、持有或賣出。
- 正式 `execution_plan`、SQLite `buy_price`／`buy_range_*`／`stop_loss`／`take_profit_*`
  必須維持空值。
- 研究數值只能存入獨立 `research_scenario` 或 validation snapshot，不得混入正式買賣欄位。
- 每個數值必須標示「未驗證、不可交易、不是買賣建議」，並列出驗證失敗狀態。
- 不得使用「建議買進價」「建議停損價」「建議停利價」等正式措辭；只能稱為情境進場、
  情境停損、情境停利或原始研究估計。
- 研究情境不得計入正式歷史交易勝率，不得被 outcome 排程視為實際交易。
...
不得為了完成格式而填入推測價格或推測勝率。只有在必要行情與計算均成功時，才可依第 2 節
的有限例外顯示獨立 `research_scenario`；資料不足時仍不得產生任何研究價位。
